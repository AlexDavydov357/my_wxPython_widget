...
from utils.common_utils import val_checker, _round, InfoMessageBox, configWriter, dev_config_reader, fig2rgb_array, resource_path2
import winsound
from utils.embed_images import _ok, _infinity_big
...
class MySliderCtrl(wx.Panel):
    def __init__(self, parent, name='', font_size=wx.DefaultSize, color=wx.WHITE, type='float', style=wx.NO_BORDER,
                 orientation='H'):
        """Универсальное поле ввода с индикатором и слайдером"""
        wx.Panel.__init__(self, parent=parent, id=-1, style=style)
        self.tooltip = ''
        self.parent = parent
        self.name = name
        self.label = name.split('_')[-1]
        self.color = color
        self.value = 0
        self.mem_value = 0
        if type == 'int':
            ...
            self.max_value = 1000
        self.orientation = 1 if orientation == 'H' else 0
        self.min_value = 0
        self.type = type
        self.increment = 1
        if type == 'float':
        ...
        if orientation == 'H':
            self.slider = wx.Slider(self, -1, self.value, self.min_value, self.max_value, name=self.name, style=wx.SL_HORIZONTAL)
            self.display = wx.StaticText(self, -1, label=f'{self.label}: 0')
            self.display.SetFont(wx.Font(wx.FontInfo(font_size)))
            ...
            self.display = wx.StaticText(self, -1, label='0', name='display', style=wx.ALIGN_CENTER)
            self.display.SetFont(wx.Font(wx.FontInfo(font_size)))
    ...
    def Menu(self):
        ...
        if self.type == 'float':
            self.Bind(wx.EVT_MENU, self.SetIncrement, id=331, id2=336)
        else:
            self.Bind(wx.EVT_MENU, self.SetIncrement, id=332, id2=336)
    ...
    def MouseWheel(self, e):
        step = round(e.GetWheelRotation() / 120, 0)  # направление
        # print(f'MouseWheel {step * self.increment}')
        self.SetValue(self.value + step * self.increment)

    def SetLabel(self, label):
        self.display.SetLabel(label)
        self.display.Layout()

    def SetIncrement(self, e):
        inc_dic = {331: 0.1, 332: 1, 333: 5, 334: 10, 335: 50, 336: 100}
        # if self.type == 'float':
        #     inc_dic = {331: 1, 332: 10, 333: 50, 334: 100, 335: 500, 336: 1000}
        print(f'SetIncrement {e.GetId()}, {inc_dic[e.GetId()]}')
    ...
    def SetMax(self, val):
        self.max_value = val
        if self.type == 'float':
            self.max_value = val * 10
        self.slider.SetMax(self.max_value)

    def SetMin(self, val):
        self.min_value = val
        if self.type == 'float':
            self.min_value = val * 10
    ...
    def SetRange(self, min_val=-1000, max_val=1000):
        ...
        if self.max_value < 10:
            # print('SetRange 1', self.name)
            self.display.SetMinSize((10, -1))
        elif self.max_value < 100:
            # print('SetRange 2', self.name)
            self.display.SetMinSize((20, -1))
        elif self.max_value < 1000:
            # print('SetRange 3', self.name)
            self.display.SetMinSize((50, -1))
        else:
            # print('SetRange 4', self.name)
            self.display.SetMinSize((60, -1))

        if self.type == 'float':
            self.min_value = min_val * 10
            self.max_value = max_val * 10
    ...
    def val_ctrl(self, e=None):
        ...
        if self.orientation:
            ...
            self.display.SetLabel(f'{self.label}: {self.round_val(cur_val)}')
        else:
            self.display.SetLabel(f'{self.round_val(cur_val)}')
        self.display.Layout()
        self.slider.Layout()
        self.Layout()
        if e is not None:
